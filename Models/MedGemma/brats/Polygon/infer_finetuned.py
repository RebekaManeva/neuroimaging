...
def extract_json(text: str) -> dict:
    ...
    if m:
        try:
            ...
            pass
    return {"has_tumor": False, "polygon_norm": [], "confidence": 0.0}


def polygon_to_mask(polygon_norm, shape=(512, 512)):
    mask = np.zeros(shape, dtype=np.uint8)
    if not polygon_norm or len(polygon_norm) < 3:
        return mask
    h, w = shape
    points = np.array([[int(p[0] * w), int(p[1] * h)] for p in polygon_norm], dtype=np.int32)
    cv2.fillPoly(mask, [points], 1)
    return mask


def compute_advanced_metrics(poly_pred, poly_gt, shape=(512, 512)):
    mask_pred = polygon_to_mask(poly_pred, shape)
    mask_gt   = polygon_to_mask(poly_gt,   shape)
    ...
    with open(jsonl_path) as f:
        for line in f:
            ...
            if not os.path.exists(r["image_path"]):
                continue
            if r.get("has_tumor") and (not r.get("polygon_norm") or len(r.get("polygon_norm", [])) < 3):
                continue
